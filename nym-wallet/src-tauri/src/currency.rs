...
impl TryFrom<CosmosDenom> for CurrencyDenom {
  ...
  fn try_from(value: CosmosDenom) -> Result<Self, Self::Error> {
    match CurrencyDenom::from_str(&value.to_string()) {
      Ok(res) => Ok(res),
      Err(_e) => Err(BackendError::InvalidDenom(value.to_string())),
    }
  }
}
...
impl MajorCurrencyAmount {
  ...
  pub fn from_cosmrs_coin(coin: &CosmosCoin) -> Result<MajorCurrencyAmount, BackendError> {
  ...
  pub fn from_minor_decimal_and_denom(
    ...
    if !(denom_minor.starts_with('u') || denom_minor.starts_with('U')) {
      return Err(BackendError::InvalidDenom(denom_minor.to_string()));
    }
    ...
    Err(BackendError::InvalidDenom(denom_minor.to_string()))
  }
  pub fn from_decimal_and_denom(
    ...
    Err(BackendError::InvalidDenom(denom))
  }
  pub fn from_cosmrs_decimal_and_denom(
    ...
    if denom.starts_with('u') || denom.starts_with('U') {
      return match Decimal::from_str(&amount.to_string()) {
        ...
        Err(_e) => Err(BackendError::InvalidAmount(amount.to_string())),
      };
    }
  ...
  pub fn into_cosmos_coin(self) -> Result<CosmosCoin, BackendError> {
  ...
  pub fn to_minor_uint128(&self) -> Result<Uint128, BackendError> {
    if self.amount.0.contains('.') {
      ...
      if str.is_empty() || str.len() > 2 {
        return Err(BackendError::InvalidAmount("Amount is invalid".to_string()));
      }
      if str.len() == 2 {
        // has a decimal, so check decimal places first
        if str[1].len() > 6 {
          return Err(BackendError::InvalidDenom(
            "Amount is invalid, only 6 decimal places of precision are allowed".to_string(),
          ));
        }
        ...
        return match format!("0.{}", str[1]).parse::<f64>() {
          ...
          Err(_e) => Err(BackendError::InvalidAmount(
  ...
  pub fn into_minor_cosmos_coin(self) -> Result<CosmosCoin, BackendError> {
  ...
  pub fn into_cosmwasm_coin(self) -> Result<CosmWasmCoin, BackendError> {
    Ok(CosmWasmCoin {
  ...
  pub fn into_minor_cosmwasm_coin(self) -> Result<CosmWasmCoin, BackendError> {
  ...
  pub fn to_string(&self) -> String {
    format!("{} {}", self.amount.0, self.denom.to_string())
  }
}
...
mod test {
  ...
  fn minor_amount_json_to_major_currency_amount() {
    ...
    let actual_nym_amount = serde_json::from_value::<MajorCurrencyAmount>(one_micro_nym).unwrap();
